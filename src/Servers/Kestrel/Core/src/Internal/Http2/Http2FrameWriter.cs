...
namespace Microsoft.AspNetCore.Server.Kestrel.Core.Internal.Http2
{
    internal class Http2FrameWriter
    {
        ...
        private readonly AspNetCoreHPackEncoder _hpackEncoder = new AspNetCoreHPackEncoder();
        ...
        */
        public void WriteResponseHeaders(int streamId, int statusCode, Http2HeadersFrameFlags headerFrameFlags, IHeaderDictionary headers)
        {
            lock (_writeLock)
            {
                ...
                try
                {
                    ...
                    var done = _hpackEncoder.BeginEncode(statusCode, new HeaderEnumerator(headers), buffer, out var payloadLength);
        ...
        public ValueTask<FlushResult> WriteResponseTrailers(int streamId, HttpResponseTrailers headers)
        {
            lock (_writeLock)
            {
                ...
                try
                {
                    ...
                    var buffer = _headerEncodingBuffer.AsSpan();
                    var done = _hpackEncoder.BeginEncode(new HeaderEnumerator(headers), buffer, out var payloadLength);
                    FinishWritingHeaders(streamId, payloadLength, done);
                }
                catch (HPackEncodingException hex)
                {
        ...
        private void FinishWritingHeaders(int streamId, int payloadLength, bool done)
        {
            ...
            while (!done)
            {
                ...
                done = _hpackEncoder.Encode1(buffer, out payloadLength);
                _outgoingFrame.PayloadLength = payloadLength;
